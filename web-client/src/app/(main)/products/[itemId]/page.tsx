...
export default function ProductDetailPage() {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            try {
                const detail = await http.get<ItemResponseType>(`/core/api/items/${itemId}`);
                setItem(detail);

                let categoryData: CategoryType | null = null;
                try {
                    categoryData = await http.get<CategoryType>(`/core/api/categories/${detail.categoryId}`);
                ...
                if (categoryData?.slug) {
                    productsInCategory = await http.get<ItemResponseType[]>(
                        `/core/api/items/category/slug/${categoryData.slug}`
                    );
                } else {
                    productsInCategory = await http.get<ItemResponseType[]>(
                        `/core/api/items/category/${detail.categoryId}`
                    );
                }

                setSimilarProducts(
                    productsInCategory.filter((product) => product.itemId !== detail.itemId).slice(0, 8)
                );
            } catch (e) {
    ...
    if (error) {
        return (
            <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
                <div className="rounded-2xl border border-red-200 bg-red-50 p-5 text-red-600">{error}</div>
            </section>
        );
    }
    ...
    return (
        <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
            ...
            <div className="mt-6 grid grid-cols-1 gap-8 lg:grid-cols-2">
                <div>
                    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-100">
                        <img src={activeImage || getPrimaryImage(undefined)} alt={item.title} className="h-full w-full object-cover" />
                    </div>
                        ...
                        <div className="mt-3 grid grid-cols-4 gap-3 sm:grid-cols-5">
                            {gallery.map((image) => (
                                <button
                                    ...
                                    className={`overflow-hidden rounded-xl border ${activeImage === image ? "border-primary" : "border-slate-200"
                                        }`}
                                >
                                    <img src={image} alt={item.title} className="h-20 w-full object-cover" />
                                </button>
                            ))}
                        </div>
                    ) : null}
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-6">
                    ...
                    <p className="mt-3 text-3xl font-extrabold text-primary">{formatPrice(item.price)}</p>

                    <div className="mt-5 space-y-3 text-sm text-slate-600">
                        <p className="inline-flex items-center gap-2">
                            <Tag size={16} />
                            Danh mục: <span className="font-medium text-slate-800">{category?.name || item.categoryId}</span>
                        </p>
                        <p className="inline-flex items-center gap-2">
                            <MapPin size={16} />
                            Địa điểm: {item.location?.district && item.location?.city
                    ...
                    <div className="mt-5 grid grid-cols-1 gap-3 rounded-xl bg-slate-50 p-4 text-sm sm:grid-cols-2">
                        <p>
                            ...
                            <span className="font-medium text-slate-800">{formatLabel(item.condition)}</span>
                        </p>
                        <p>
                            <span className="text-slate-500">Loại giao dịch:</span>{" "}
                            <span className="font-medium text-slate-800">{formatLabel(item.transactionType)}</span>
                        </p>
                        <p>
                            <span className="text-slate-500">Trạng thái:</span>{" "}
                            <span className="font-medium text-slate-800">{formatLabel(item.status)}</span>
                        </p>
                        <p>
            ...
            <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-6">
                <h2 className="text-lg font-semibold text-slate-900">Thuộc tính chi tiết</h2>
                {!item.attributes || item.attributes.length === 0 ? (
                    <p className="mt-3 text-slate-500">Sản phẩm chưa có thuộc tính bổ sung.</p>
                ) : (
                    <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                        {item.attributes.map((attribute) => (
                            <div key={attribute.code} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                                <p className="text-xs uppercase tracking-wide text-slate-500">{attribute.code}</p>
                                <p className="mt-1 font-medium text-slate-800">{String(attribute.value)}</p>
                            </div>
                        ))}
                    </div>
                )}
            ...
            <div className="mt-10">
                ...
                <p className="mt-1 text-sm text-slate-500">Các sản phẩm cùng danh mục với tin này.</p>
