...
export default function ItemDetailPage() {
      ...
      .then((data) => {
        ...
        if (data.categoryId) {
          categoryService
            ...
            .catch(() => {});
          itemService
            .searchItems({ categoryId: data.categoryId, size: 5 })
            .then((res) =>
              setSimilarItems(
                res.content.filter((i) => i.itemId !== data.itemId).slice(0, 4),
              ),
            )
            .catch(() => {});
  ...
  if (!item) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
        ...
        <Link
          href="/home"
          className="bg-emerald-500 text-white px-6 py-2.5 rounded-xl font-bold hover:bg-emerald-600 transition inline-flex items-center gap-2"
        >
  ...
  const images =
    item.images && item.images.length > 0
      ...
      : [
          {
            imageUrl: "/icon-other/san-pham-khac.png",
            isPrimary: true,
            imageId: "default",
          },
        ];
  ...
  const handleChat = async () => {
    ...
    try {
      ...
      if (res.conversationId) {
        ...
        if (sellerProfile?.user_profile?.fullName)
          params.set("name", sellerProfile.user_profile.fullName);
        if (sellerProfile?.user_profile?.avatarUrl)
          params.set("avatar", sellerProfile.user_profile.avatarUrl);
        router.push(`/chat/${res.conversationId}?${params.toString()}`);
      }
    } catch {
      messageApi.error("Lỗi khi tạo cuộc trò chuyện");
    } finally {
  ...
  const sellerName =
    sellerProfile?.user_profile?.fullName ||
    "Người bán " + (item.userId?.substring(0, 6) || "");
            ...
            {images.length > 1 && (
              <>
                <button
                  ...
                  className="absolute left-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center text-white backdrop-blur-sm z-10"
                >
                ...
                <button
                  ...
                  className="absolute right-4 top-1/2 -translate-y-1/2 w-12 h-12 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center text-white backdrop-blur-sm z-10"
                >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-7 space-y-6">
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                {/* Main Image */}
                <div
                  className="relative aspect-[4/3] bg-slate-100 group cursor-pointer"
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
                <h2 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
                  <Tag className="w-5 h-5 text-emerald-600" /> Thông tin sản
                  phẩm
                </h2>
                ...
                <div>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <Link
                      ...
                      className="text-emerald-600 text-sm font-semibold hover:underline flex items-center gap-0.5"
                    >
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {similarItems.map((simItem) => (
                      ...
                      >
                        ...
                        <div className="p-3">
                          <h3 className="line-clamp-2 text-sm font-medium text-slate-800 group-hover:text-emerald-600 transition-colors min-h-[36px]">
                            {simItem.title}
                          </h3>
                          <p className="mt-2 text-base font-black text-emerald-600">
                            {formatPrice(simItem.price)}
                          </p>
                        </div>
                      </Link>
                    ))}
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                  <FavoriteButton
                    ...
                    showCount={true}
                    showLabel={true}
                    className="text-slate-400 hover:text-red-500 bg-slate-50 hover:bg-red-50 transition p-2 px-4 rounded-xl"
                  />
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => openReportModal(item.itemId)}
                      className="text-slate-400 hover:text-red-600 bg-slate-50 hover:bg-red-50 transition p-2 px-4 rounded-xl flex items-center gap-2 text-sm font-semibold"
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <Link
                    ...
                    className="text-sm font-bold text-emerald-600 hover:text-emerald-700 transition hover:underline shrink-0"
                  >
                    Xem trang →
                  </Link>
                </div>
                <div className="flex border-t border-slate-100 pt-4 text-center divide-x divide-slate-100">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="font-black text-slate-800 flex items-center justify-center gap-1">
                      <Clock className="w-3.5 h-3.5 text-emerald-500" /> ~15p
                    </p>
                  </div>
                </div>
              </div>
