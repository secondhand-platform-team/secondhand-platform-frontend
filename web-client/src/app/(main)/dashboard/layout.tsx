...
import { LayoutDashboard, ShoppingBag, Users, Settings, KeyRound, Menu, X, CheckCircle2, MapPin, Heart } from "lucide-react";
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const navigation = [
    ...
    { name: "Quản lý tin đăng", href: "/dashboard/my-posts", icon: ShoppingBag },
  ...
  const profileStats = [
    ...
    { label: "Đang rao bán", value: user?.freeSellUsed || "12", highlight: false },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-3xl overflow-hidden shadow-sm border border-slate-100 mb-8 relative">
          ...
          <div className="relative h-36 sm:h-40 overflow-hidden">
            ...
            {/* SVG Wave Bottom */}
            <svg className="absolute bottom-0 left-0 w-full" viewBox="0 0 1440 80" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none" style={{ height: '40px' }}>
            ...
            <div className="absolute top-4 right-4 z-10">
              <Link href="/dashboard/profile" className="px-5 py-2 bg-white/90 hover:bg-white text-emerald-700 rounded-xl font-bold text-sm transition-all shadow-lg backdrop-blur-sm flex items-center gap-2">
          ...
          <div className="px-6 pb-6 pt-2 relative flex flex-col md:flex-row md:items-end md:justify-between gap-6 -mt-16">
            <div className="flex flex-col md:flex-row items-center md:items-end gap-6">
              ...
              <div className="text-center md:text-left mb-2">
                <h1 className="text-3xl font-black text-slate-900">{user?.fullName || "Người dùng"}</h1>
                <div className="flex items-center justify-center md:justify-start gap-4 text-sm text-slate-500 mt-2">
                  <span className="flex items-center gap-1.5">
                    <MapPin className="w-4 h-4" /> 
                    {user?.city ? `${user.district ? user.district + ", " : ""}${user.city}` : "Toàn quốc"}
                  </span>
                  <span className="flex items-center gap-1.5"><ShoppingBag className="w-4 h-4" /> Tham gia từ T10/2023</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center md:items-end gap-3 mb-2">
              <div className="flex items-end gap-3">
                <span className="text-4xl font-black text-emerald-600">4.8</span>
                <div className="pb-1">
                  <Rate disabled defaultValue={5} className="text-amber-400 text-sm" />
                  <p className="text-xs text-slate-500 mt-0.5">(128 đánh giá)</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          {/* Sidebar */}
          <div className={`lg:w-72 shrink-0 space-y-6 ${mobileMenuOpen ? 'block' : 'hidden lg:block'}`}>

            {/* Navigation */}
            <div className="bg-white rounded-3xl p-4 shadow-sm border border-slate-100">
              <nav className="space-y-1.5">
                {navigation.map((item) => {
                  const isActive = pathname === item.href || (pathname.startsWith(item.href) && item.href !== '/dashboard' && item.href !== '/my-posts');
                  return (
                    <Link
                      key={item.name}
                      href={item.href}
            ...
            <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
              <div className="flex items-center gap-2 mb-4">
                <div className="bg-emerald-100 text-emerald-600 p-1.5 rounded-lg"><LayoutDashboard className="w-4 h-4" /></div>
              ...
              <div className="space-y-4">
                {profileStats.map((stat, idx) => (
                  <div key={idx} className="flex justify-between items-center text-sm">
                    ...
                    <span className={`font-bold ${stat.highlight ? 'text-emerald-600' : 'text-slate-800'}`}>{stat.value}</span>
                  </div>
                ))}
              </div>
            </div>
