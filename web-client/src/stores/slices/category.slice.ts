...
>("category/fetchTopCategories", async (_, { rejectWithValue }) => {
  try {
    const response = await http.get<CategoryType[]>(
      "core/api/categories/top-level",
    );
    return Array.isArray(response) ? response : response.data || [];
  } catch (error) {
    return rejectWithValue(
      getErrorMessage(error, "Không thể tải danh mục cấp cao nhất"),
    );
  }
});
...
>("category/fetchParentCategoryBySlug", async (slug, { rejectWithValue }) => {
  try {
    return await http.get<CategoryType>(
      `core/api/categories/slug/${slug}`,
    );
  } catch (error) {
    return rejectWithValue(
      getErrorMessage(error, "Không thể tải thông tin danh mục"),
    );
  }
});
...
>("category/fetchChildCategoryBySlug", async (slug, { rejectWithValue }) => {
  try {
    ...
    return rejectWithValue(
      getErrorMessage(error, "Không thể tải thông tin danh mục con"),
    );
  }
});
...
export const fetchChildCategoriesByParentSlug = createAsyncThunk<
  ...
  { rejectValue: string }
>(
  "category/fetchChildCategoriesByParentSlug",
  async (slug, { rejectWithValue }) => {
    try {
      const response = await http.get<CategoryType[]>(
        `core/api/categories/slug/${slug}/children`,
      );
      return Array.isArray(response) ? response : response.data || [];
    } catch (error) {
      return rejectWithValue(
        getErrorMessage(error, "Không thể tải danh mục con"),
      );
    }
  },
);
...
export const fetchProductsByCategorySlug = createAsyncThunk<
  ...
  { rejectValue: string }
>(
  "category/fetchProductsByCategorySlug",
  async (slug, { rejectWithValue }) => {
    try {
      const response = await http.get<ItemResponseType[]>(
        `core/api/items/category/slug/${slug}`,
      );
      return Array.isArray(response) ? response : response.data || [];
    } catch (error) {
      return rejectWithValue(
        getErrorMessage(error, "Không thể tải sản phẩm theo danh mục"),
      );
    }
  },
);
...
export const fetchProductsByParentAndChildrenSlug = createAsyncThunk<
  ...
  { rejectValue: string }
>(
  "category/fetchProductsByParentAndChildrenSlug",
  async (parentSlug, { rejectWithValue }) => {
    try {
      const parentResponse = await http.get<CategoryType>(
        `core/api/categories/slug/${parentSlug}`,
      );
      const parent = !Array.isArray(parentResponse) ? parentResponse : parentResponse[0];
      
      const childrenResponse = await http.get<CategoryType[]>(
        `core/api/categories/slug/${parentSlug}/children`,
      );
      ...
      const productsBySlug = await Promise.all(
        slugs.map((slug) =>
          http.get<ItemResponseType[]>(
            `core/api/items/category/slug/${slug}`,
          ),
        ),
      );

      const flatProducts = productsBySlug.flat();
      const merged = flatProducts.every(Array.isArray)
        ? flatProducts.flat()
        : flatProducts;
      const uniqueById = new Map(
        merged.map((item) => [item.itemId, item]),
      );

      return Array.from(uniqueById.values());
    } catch (error) {
      return rejectWithValue(
        getErrorMessage(error, "Không thể tải sản phẩm của danh mục"),
      );
    }
  },
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchTopCategories.rejected, (state, action) => {
        state.loading = false;
        state.error =
          action.payload || "Không thể tải danh mục cấp cao nhất";
      })
      ...
      .addCase(fetchParentCategoryBySlug.rejected, (state, action) => {
        state.loading = false;
        state.error =
          action.payload || "Không thể tải thông tin danh mục";
      })
      ...
      .addCase(fetchChildCategoryBySlug.rejected, (state, action) => {
        state.loading = false;
        state.error =
          action.payload || "Không thể tải thông tin danh mục con";
      })
      ...
      .addCase(fetchProductsByCategorySlug.rejected, (state, action) => {
        state.loading = false;
        state.error =
          action.payload || "Không thể tải sản phẩm theo danh mục";
      })
      ...
      .addCase(
        fetchProductsByParentAndChildrenSlug.fulfilled,
        (state, action) => {
          state.loading = false;
      ...
      .addCase(
        fetchProductsByParentAndChildrenSlug.rejected,
        (state, action) => {
          state.loading = false;
          state.error =
            action.payload || "Không thể tải sản phẩm của danh mục";
        },
      );
  },
});
