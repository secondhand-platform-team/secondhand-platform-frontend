...
type ProductCardProps = {
  ...
  badge?: string;
  isFavorited?: boolean;
  onFavoriteToggle?: (itemId: string, isFavorited: boolean) => Promise<void>;
  onViewDetails?: () => void;
};
...
export default function ProductCard({
  ...
  const handleFavoriteClick = async (e: React.MouseEvent) => {
    ...
    if (id && onFavoriteToggle) {
      await onFavoriteToggle(id, !isFavorited);
    }
  };
  ...
  return (
    <article className="overflow-hidden rounded-[28px] border border-white/80 bg-white shadow-[0_18px_50px_rgba(15,23,42,0.06)]">
      <div className="relative aspect-[1/1.05] overflow-hidden bg-slate-100">
        ...
        <button
          type="button"
          className="absolute right-4 top-4 inline-flex h-10 w-10 items-center justify-center rounded-full bg-white/90 text-slate-700 shadow-sm transition hover:bg-primary hover:text-white"
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-3">
          ...
          <span className="whitespace-nowrap text-lg font-bold text-emerald-500">
            {price}
          </span>
        </div>
